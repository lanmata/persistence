/*
 * @(#)ServiceTypeRepository.java.
 *
 * Copyright (c) Luis Antonio Mata Mata. All rights reserved.
 *
 * All rights to this product are owned by Luis Antonio Mata Mata and may only
 * be used under the terms of its associated license document. You may NOT
 * copy, modify, sublicense, or distribute this source file or portions of
 * it unless previously authorized in writing by Luis Antonio Mata Mata.
 * In any event, this notice and the above copyright must always be included
 * verbatim with this file.
 */
package com.umdc.persistence.general.repositories;

import com.umdc.persistence.general.domains.ServiceTypeEntity;
import org.springframework.data.repository.CrudRepository;

import java.util.UUID;

/**
 * Repository interface for accessing and managing {@link ServiceTypeEntity} data.
 * This interface provides CRUD operations for {@link ServiceTypeEntity} objects
 * with a primary key of type {@link UUID}.
 *
 * It extends {@link CrudRepository}, inheriting methods to save, find, delete,
 * and manage {@link ServiceTypeEntity} objects in the persistence layer.
 *
 * @author <a href='mailto:luis.antonio.mata@gmail.com'>Luis Antonio Mata.</a>
 * @version 1.0.3.20200904-01, 19-01-2021
 */
public interface ServiceTypeRepository extends CrudRepository<ServiceTypeEntity, UUID> {

}
